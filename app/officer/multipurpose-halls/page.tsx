...
export default function MultipurposeHallsPage() {
  ...
  const { halls, meta, loading, error, page, setPage } = useMultipurposeHalls(selectedDistrictId || undefined);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">
              {showForm ? 'Register New Multipurpose Hall' : 'Multipurpose Halls'}
            </h2>
            {showForm && <p className="text-xs text-gray-500 mt-0.5">Define facility details, POC, and available infrastructure</p>}
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                ...
                <p className="text-sm">Loading halls...</p>
              </div>
            ) : error ? (
              ...
              <div className="text-center py-20 text-gray-500">
                <i className="fas fa-building text-3xl mb-3 text-gray-200"></i>
                <p className="text-sm">No multipurpose halls found.</p>
              </div>
            ) : (
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50 border-b border-gray-200 text-left">
                      ...
                      <th className="px-6 py-3 font-semibold text-gray-700 uppercase tracking-wider text-[11px]">Capacity</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {halls.map((h) => (
                      <tr key={h.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 text-gray-900 font-medium">{h.name}</td>
                        <td className="px-6 py-4 text-gray-600">{h.location || '—'}</td>
                        <td className="px-6 py-4 text-gray-600 font-mono text-[13px]">
                          {h.capacity ? `${h.capacity} persons` : '—'}
                        </td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`text-[11px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${
                            h.isActive ? 'bg-green-100 text-green-700 shadow-sm' : 'bg-red-100 text-red-600 shadow-sm'
                          }`}>
