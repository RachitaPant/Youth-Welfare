...
export default function IndoorGymPage() {
  ...
  const { gyms, meta, loading, error, page, setPage } = useIndoorGyms(selectedDistrictId || undefined);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">
              {showForm ? 'Register New Indoor Gym' : 'Indoor Gyms'}
            </h2>
            {showForm && <p className="text-xs text-gray-500 mt-0.5">Define gym facilities, equipment, and POC</p>}
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            {loading ? (
              ...
              <div className="text-center py-20 text-gray-500">
                <i className="fas fa-dumbbell text-3xl mb-3 text-gray-200"></i>
                <p className="text-sm">No indoor gyms found.</p>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {gyms.map((g) => (
                      <tr key={g.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-6 py-4 text-gray-900 font-medium">{g.name}</td>
                        ...
                        <td className="px-6 py-4">
                          <span className={`text-[11px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${
                            g.isActive ? 'bg-green-100 text-green-700 shadow-sm' : 'bg-red-100 text-red-600 shadow-sm'
                          }`}>
