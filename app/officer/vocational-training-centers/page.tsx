...
export default function VocationalTrainingCentersPage() {
  ...
  const { centers, meta, loading, error, page, setPage } = useVocationalCenters(selectedDistrictId || undefined);
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-bold text-gray-800">
              {showForm ? 'Register New Training Center' : 'Vocational Training Centers'}
            </h2>
            {showForm && <p className="text-xs text-gray-500 mt-0.5">Define center details, courses offered, and POC</p>}
          </div>
        </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 text-gray-400">
                ...
                <p className="text-sm">Loading centers...</p>
              </div>
            ) : error ? (
              ...
              <div className="text-center py-20 text-gray-500">
                <i className="fas fa-graduation-cap text-3xl mb-3 text-gray-200"></i>
                <p className="text-sm">No vocational training centers found.</p>
              </div>
            ) : (
                      ...
                      <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`text-[11px] px-2.5 py-1 rounded-full font-bold uppercase tracking-wider ${
                            c.isActive ? 'bg-green-100 text-green-700 shadow-sm' : 'bg-red-100 text-red-600 shadow-sm'
                          }`}>
